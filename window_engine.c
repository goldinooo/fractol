...
/*   Updated: 2025/03/22 02:23:44 by retahri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	reset_win(t_win *win, int frac_type)
{
	...
	if (frac_type == JUL)
	{
		...
		win->fractal.n.real = -0.8;
		win->fractal.n.imag = 0.156;
		win->fractal.iterations = 200;
		win->fractal.off_x = 0.0;
...
void	launch(t_win *win, char *av)
{
	...
	if (!(win->window = mlx_new_window(win->mlx, WIDTH, HIGHT,
				"psychedelic Thinker")))
		clearup(win);
